...
- Migrations: `supabase/migrations/20260917090000_schema.sql` (6 tabel + RLS + bucket `avatars`), `20260917090001_seed_system_categories.sql` (12 kategori sistem, idempotent), `20260918090000_wallet_balances_and_reassign.sql` (T4: view saldo + RPC reassign).
...
- Views agregasi: `v_wallet_balances` ada sejak T4 (#5). `v_monthly_summary`, `v_category_breakdown`, `v_budget_status`, dan `current_month(tz)` **belum ada** — milik T6/T7 sesuai `specs/tickets.md`.
...
- State remote (sudah diverifikasi): 6 tabel, 24 policy public, 4 policy `avatars_*` di `storage.objects`, bucket privat 2MB PNG/JPG, 12 kategori sistem, 1 view `v_wallet_balances` (security_invoker) + 1 RPC `reassign_wallet_transactions`, 118 assertion pgTAP hijau (dijalankan via `psql` karena mesin ini tanpa Docker).
...
- DB tests (pgTAP): `npm run db:test` (`supabase test db`). **Butuh Docker/Podman** — `supabase test db --linked` tetap menjalankan `pg_prove` di container. Suite di `supabase/tests/database/` (9 file, 118 assertion) dan bisa juga dijalankan langsung dengan `psql` terhadap DB mana pun (`00_setup.sql` dulu, lalu 01–08; `set role postgres` + `set search_path = public, extensions`).
...
- Verifikasi state DB tanpa Docker (pola T3/T4): uji end-to-end lewat klien anon (signup → `seed-user` → baca lewat RLS) lalu periksa hasilnya via MCP `execute_sql` / `psql`. Selalu bersihkan user uji (`delete from auth.users where email like 't3-%@cashtrix.test'` / `'t4-%@cashtrix.test'`) — cascade menghapus profile + wallet + transaksinya, dan verifikasi 0 residu setelahnya.
